...
test("sandboxed bash reads git history and .env but cannot change either, or reach home", { skip: !SandboxManager.isSupportedPlatform() }, async t => {
  ...
  t.after(() => Promise.all([rm(repo, { recursive: true, force: true }), rm(home, { recursive: true, force: true })]));
  const env = Object.fromEntries(Object.entries(process.env).filter(([name]) => !name.startsWith("GIT_")));
  const git = (...args: string[]) => promisify(execFile)("git", ["-C", repo, "-c", "user.name=t", "-c", "user.email=t@t", ...args], { env });
  await writeFile(join(repo, "tracked.txt"), "one\n");
  await git("init", "--quiet"); await git("add", "."); await git("commit", "--quiet", "-m", "init");
  ...
  for (const command of ["git add .", "git -c user.name=t -c user.email=t@t commit -qam x", "git stash", "git reset --hard -q", "git checkout -- ."]) {
