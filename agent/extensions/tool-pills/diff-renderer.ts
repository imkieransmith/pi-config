...
import { createEditTool, createWriteTool, keyHint } from "@earendil-works/pi-coding-agent";
...
function isLightRgb(rgb: { r: number; g: number; b: number }): boolean {
	return luminance(rgb) > 170;
}
...
function applyDiffPalette(): void {
	...
	if (shiki) {
		THEME = shiki as BundledTheme;
		_hasExplicitShikiTheme = true;
	}
...
let _hasExplicitShikiTheme = !!process.env.DIFF_THEME;
...
function themeBgRgb(theme?: any): { r: number; g: number; b: number } | null {
	const activeBase = parseAnsiRgb(BG_BASE);
	if (activeBase) return activeBase;

	const activeAdd = parseAnsiRgb(BG_ADD);
	if (activeAdd) return activeAdd;

	if (!theme?.getBgAnsi) return null;
	for (const role of ["toolSuccessBg", "background", "toolBg"]) {
		try {
			const parsed = parseAnsiRgb(theme.getBgAnsi(role));
			if (parsed) return parsed;
		} catch {
			/* try next role */
		}
	}
	return parseAnsiRgb(BG_BASE);
}

function syncShikiThemeToBackground(theme?: any): void {
	if (_hasExplicitShikiTheme) return;
	const bg = themeBgRgb(theme);
	if (!bg) return;

	const next = (isLightRgb(bg) ? "github-light" : "github-dark") as BundledTheme;
...
function resolveDiffColors(theme?: any): DiffColors {
	...
	syncShikiThemeToBackground(theme);
...
function basicAnsiFgLuminance(params: string): number | null {
	...
	if (params in basic) return basic[params];
	...
	return indexed[params] ?? null;
}

function shikiFgLuminance(params: string): number | null {
	const basic = basicAnsiFgLuminance(params);
	if (basic !== null) return basic;
	if (!params.startsWith("38;2;")) return null;
...
function contrastBgRgb(): { r: number; g: number; b: number } | null {
	return parseAnsiRgb(BG_BASE) ?? parseAnsiRgb(BG_ADD);
}

function isLowContrastShikiFg(params: string): boolean {
	const fgLum = shikiFgLuminance(params);
	if (fgLum === null) return false;

	const bg = contrastBgRgb();
	const lightBg = bg ? isLightRgb(bg) : false;
	return lightBg ? fgLum > 190 : fgLum < 72;
}

function safeShikiFg(): string {
	const bg = contrastBgRgb();
	return bg && isLightRgb(bg) ? "\x1b[38;2;36;41;47m" : FG_SAFE_MUTED;
}

function normalizeShikiContrast(ansi: string): string {
	return ansi.replace(ANSI_PARAM_CAPTURE_RE, (seq, params: string) =>
		isLowContrastShikiFg(params) ? safeShikiFg() : seq,
	);
}
...
export function registerDiffTools(pi: any): void {
	...
	const createWriteFn = createWriteTool;
	const createEditFn = createEditTool;
	const TextComponent = Text;
	const pillFn = pill;
	const keyHintFn = keyHint;
	...
	pi.registerTool({
		...
		async execute(tid: string, params: any, sig: any, upd: any, ctx: any) {
			...
			try {
				if (fp) {
					const abs = resolve(ctx.cwd ?? cwd, fp);
					if (existsSync(abs)) old = readFileSync(abs, "utf-8");
				}
			} catch {
				old = null;
			}
		...
		renderCall(args: any, theme: any, ctx: any) {
			...
			const abs = fp ? resolve(ctx.cwd ?? cwd, fp) : "";
			const isNew = !!fp && !existsSync(abs);
	...
	pi.registerTool({
		...
		async execute(tid: string, params: any, sig: any, upd: any, ctx: any) {
			...
			try {
				if (fp) {
					const abs = resolve(ctx.cwd ?? cwd, fp);
					if (existsSync(abs)) preEditContent = readFileSync(abs, "utf-8");
				}
			} catch {
				preEditContent = null;
			}
