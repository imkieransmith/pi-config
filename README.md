...
- **Sandbox** (`sandbox/`): Owns the bash tool and runs the agent's bash commands in an OS sandbox through [sandbox-runtime](https://github.com/anthropics/sandbox-runtime). Home stays hidden except the project and tool folders, writes stay in the project and `/tmp`, and network access is limited to a short list of domains and localhost. The project's `.git` and `.env` are read-only, so agents can read history but not commit, reset, stash or push. Your own `!` commands run as normal. Both use `rtk rewrite` to shorten some shell output when RTK supports the command. Linux needs `bubblewrap`, `socat`, and `ripgrep`.
- **Security Guard** (`security/`): Keeps Pi's own file tools (read, write, edit, grep, find, ls) away from secrets and private files. Owns the protected grep, find, and ls tools, including their compact rows. Search protection stays active without Tool Pills. Bash is left to the sandbox.
- **Standalone Working** (`standalone-working.ts`): Shows working status above the input, not inside it.
- **Tool Pills** (`tool-pills`): Adds compact read rows and write/edit diffs; click to expand details/images. Security and Sandbox own their tool rows using helpers in `shared/`. Optional: bash and grep rows show a plain-English sentence if you set `explain.model` in settings.
